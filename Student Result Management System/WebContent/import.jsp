...
<body>
	...
	if ("students".equals(request.getParameter("check"))) {
		...
		if (!f.exists()) {
			response.sendRedirect("InstituteHome.jsp?I_ID=" + request.getParameter("I_ID") + "&alert=File Not Found.");
		} else {
		...
		st.executeUpdate();
			...
			"InstituteHome.jsp?I_ID=" + request.getParameter("I_ID") + "&sucsses=Data Added Sucssesfully.");
		}
		...
		"InstituteHome.jsp?I_ID=" + request.getParameter("I_ID") + "&sucsses=Data Added Sucssesfully.");
			...
			out.print(ex);
			//response.sendRedirect("InstituteHome.jsp?I_ID=" + request.getParameter("I_ID") + "&alert=Invalid Data in File.");
		}try{if ("institutes".equals(request.getParameter("check"))) {
		File f = new File(request.getParameter("path"));
		if (!f.exists()) {
			response.sendRedirect("AdminHome2.jsp?&alert=File Not Found.");
		}else{
		...
		con.close();
		response.sendRedirect("AdminHome2.jsp?sucsses=Data Exported.");
		}}}catch(Exception ex){
			
			response.sendRedirect("AdminHome2.jsp?&alert=Invalid Data in File.");
		}
	%>
</body>
</html>